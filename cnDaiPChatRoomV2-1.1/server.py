...
class RegisterHandler(BaseHandler):
    def post(self):
        try:
            ...
            self.write({"success": False, "message": f"注册失败: {str(e)}"})
    ...
    def post(self):
        try:
            ...
            if result and result[0] == password:
                self.write({"success": True, "message": "登录成功", "username": username})
            else:
                self.write({"success": False, "message": "用户名或密码错误"})
        except Exception as e:
            self.write({"success": False, "message": f"登录失败: {str(e)}"})
    ...
    def on_message(self, message):
        ...
        try:
            ...
            if msg_type == "music_control":
                print(f"Music control message: {data}")
                self.broadcast(data)
                return
            ...
            elif content.startswith("@听音乐") or content.startswith("@音乐"):
                # Format: @听音乐 关键词 或 @音乐 关键词
                print(f"Music command: {content}")
                # 使用新的音乐API获取直接MP3链接
                import requests
                
                # 固定URL模板和API
                MUSIC_API = "https://api.uomg.com/api/rand.music"
                
                # 从指令中提取关键词
                keyword = content[4:].strip() if content.startswith("@音乐") else content[5:].strip()
                print(f"Music keyword: {keyword}")
                
                if keyword:
                    try:
                        # 调用API搜索歌曲，获取直接MP3链接
                        response_api = requests.get(f"{MUSIC_API}?sort=热歌榜&format=json&keyword={keyword}", timeout=5)
                        result = response_api.json()
                        print(f"Music API result: {result}")
                        
                        if result.get("code") == 1:
                            # 提取歌曲信息
                            song_name = result.get("name", "未知歌曲")
                            artist = result.get("artistsname", "未知歌手")
                        ...
                        response = {
                            "type": "text",
                            "sender": self.nickname,
                            "content": f"搜索过程中发生错误：{str(e)}",
                            "timestamp": data.get("timestamp")
                        }
                else:
                    # 未提供关键词，获取随机推荐
                    try:
                        response_api = requests.get(f"{MUSIC_API}?sort=热歌榜&format=json", timeout=5)
                        result = response_api.json()
                        print(f"Random music API result: {result}")
                        ...
                        # 网络错误或其他异常
                        print(f"Random music API error: {e}")
                        response = {
                            "type": "text",
                            "sender": self.nickname,
                            "content": f"随机推荐过程中发生错误：{str(e)}",
                            "timestamp": data.get("timestamp")
                        }
            else:
